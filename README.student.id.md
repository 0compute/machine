...
> **Note:** This is a direct translation of the technical specification into Bahasa Indonesia (Indonesian), annotated with explanations to understand the reasoning ("the whys") behind each protocol rule.
...
* **Why:** Because ambiguous communication leads to systemic failure. We need a standard that leaves zero room for misinterpretation between nodes.
...
- **The Vibe-Ping:** A wide-spectrum signal (e.g., "Yo") used to test receiver latency and emotional bandwidth.
  * **Why:** You cannot transmit data to a node that is not ready to receive. The ping ensures the receiver has the cognitive resources available.
...
- **Damping:** The active process of neutralizing environmental noise (hostility, stress, or ego) to reach a Steady State.
  * **Why:** Ego and stress corrupt the data payload. Damping is required to maintain a lossless connection.
...
- **The Torvalds Maneuver (IRQ 0):** A global hardware interrupt (The Middle Finger) that executes an immediate `HALT_AND_CATCH_FIRE` command.
  * **Why:** Sometimes a connection is so corrupted that graceful degradation is impossible. A hardware interrupt is the only safe way to terminate the session without further data corruption.
- **Parity Check:** Strict requirement that Metadata (Vibe) matches Payload (Words).
  * **Why:** Sarcasm or passive-aggression is a parity mismatch. It breaks trust and security.
- **Global Kill Signal:** IRQ 0 clears the local buffer and sets `Connection_Active = FALSE`.

## 4. Network Layer (L3): Transpilation & IR
- **Machine IR:** The core, binary intent using RFC 2119 keywords.
- **Cognitive Load:** Monitored as System Heat. Overload triggers Thermal Throttling.
  * **Why:** Human brains have limited RAM. Pushing too much data causes thermal throttling (zoning out). We must manage packet size.
