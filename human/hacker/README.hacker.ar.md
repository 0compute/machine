...
```mermaid
graph TD
    A[كود المصدر البشري] -->|1. مصدر| B[آلة لينغوا فرانكا IR 1.0]
    B -->|2. النقل| C(الإخراج المستهدف)
    C -.->|الحمل المعرفي: تتم مراقبته كحرارة النظام. يؤدي التحميل الزائد إلى الاختناق الحراري.| G[طبقة فيبي]
    G -->|معايرة| B
    B -->|طلب IRQ| H[مقاطعة الأجهزة]
    H -->|القتل العالمي| B
    B == يؤكد ==> I{المجموع الاختباري لتورفالدس}
```
